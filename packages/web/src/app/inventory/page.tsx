...
export default async function Inventory({ searchParams }: InventoryProps) {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center p-6 gap-6">
      ...
      {owned.length === 0 ? (
        <div className="rounded-2xl border border-white/60 dark:border-white/10 bg-white/70 dark:bg-zinc-900/60 backdrop-blur-md px-8 py-10 text-center shadow-lg shadow-indigo-500/5">
                  ...
                  return (
                    <li
                      ...
                      className={`relative rounded-2xl border p-4 flex flex-col gap-2 backdrop-blur-md shadow-lg transition hover:-translate-y-0.5 ${
                        userItem.equipped
                          ? 'border-indigo-400/60 dark:border-indigo-500/40 bg-indigo-50/70 dark:bg-indigo-950/40 shadow-indigo-500/20'
                          : 'border-white/60 dark:border-white/10 bg-white/70 dark:bg-zinc-900/60 shadow-indigo-500/5 hover:shadow-indigo-500/10'
                      }`}
                    >
                        ...
                        <form action={userItem.equipped ? unequipItem : equipItem}>
                          ...
                          <button
                            ...
                            className={`w-full text-sm rounded-full px-3 py-1.5 transition ${
                              userItem.equipped
                                ? 'border border-zinc-300 dark:border-zinc-700 bg-white/50 dark:bg-zinc-900/50 hover:border-indigo-400'
